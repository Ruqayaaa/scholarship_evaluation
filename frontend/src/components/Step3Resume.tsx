...
export function Step3Resume({ data, onUpdate, onNext, onBack, onSaveDraft }: Step3Props) {
  ...
  async function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!text) { setOcrError("No text could be extracted. Make sure the PDF contains selectable text (not a scanned image)."); }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div className="card" style={{ width: "100%" }}>
        ...
        <div style={{ color: "var(--muted)", lineHeight: 1.6, marginBottom: 14 }}>
          Upload a <strong>PDF</strong> of your resume — text will be extracted so you can review it and fill the fields below. Only text-based PDFs are supported.
        </div>
